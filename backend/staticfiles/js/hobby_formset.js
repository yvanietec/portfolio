// Handles dynamic add/remove for hobby formset

document.addEventListener('DOMContentLoaded', function () {
    console.log('Hobby formset JavaScript loaded');
    ...
    if (!addBtn || !formsContainer || !totalForms) {
        console.log('Hobby formset elements not found');
        return;
    }
        ...
        const hasData = Array.from(card.querySelectorAll('input, select, textarea')).some(function (el) {
            if (el.type === 'checkbox' || el.type === 'radio') {
                ...
                if (value === '' || value === el.placeholder ||
                    value === 'e.g., Painting, Reading, Hiking') {
                    return false;
                }
                return true;
            }
    ...
    addBtn.addEventListener('click', function (e) {
        console.log('Add hobby button clicked!');
    ...
    formsContainer.addEventListener('click', function (e) {
        if (e.target.closest('.remove-entry, .remove-btn')) {
            console.log('Remove hobby button clicked');
            ...
            if (totalCards > 1) {
                ...
                // Show a subtle message that at least one hobby entry is required
    ...
    console.log('Hobby formset JavaScript initialization complete');
});
