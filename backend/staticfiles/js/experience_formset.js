// Handles dynamic add/remove for experience formset

document.addEventListener('DOMContentLoaded', function () {
    console.log('Experience formset JavaScript loaded');
    ...
    console.log('Experience formset elements:', {
        addBtn: addBtn,
    ...
    if (!addBtn || !formsContainer || !totalForms) {
        console.log('Experience formset elements not found');
        return;
    }
    ...
    function reindexForms() {
        console.log('Reindexing experience forms');
        const cards = formsContainer.querySelectorAll('.portfolio-form-section');
        console.log('Found experience cards:', cards.length);

        cards.forEach((card, idx) => {
            // Update card title
            const cardTitle = card.querySelector('.portfolio-section-title');
            if (cardTitle) cardTitle.textContent = `Experience #${idx + 1}`;
        ...
        const hasData = Array.from(card.querySelectorAll('input, select, textarea')).some(function (el) {
            if (el.type === 'checkbox' || el.type === 'radio') {
                ...
                if (value === '' || value === el.placeholder ||
                    ...
                    value === 'e.g. Infosys, Google' ||
                    value === 'e.g. Bengaluru, India or Remote' ||
                    value === 'Briefly describe your role, tools used, and achievements.') {
                    return false;
                }
                return true;
            }
    ...
    formsContainer.addEventListener('click', function (e) {
        if (e.target.closest('.remove-entry, .remove-btn')) {
            ...
            if (totalCards > 1) {
                ...
                // Show a subtle message that at least one experience entry is required
