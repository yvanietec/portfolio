// Handles dynamic add/remove for language formset

document.addEventListener('DOMContentLoaded', function () {
    console.log('Language formset JavaScript loaded');
    ...
    if (!addBtn || !formsContainer || !totalForms) {
        console.log('Language formset elements not found');
        return;
    }
    ...
    function reindexForms() {
        console.log('Reindexing language forms');
        ...
        cards.forEach((card, idx) => {
            ...
            if (cardTitle) cardTitle.textContent = `Language #${idx + 1}`;
    ...
    function updateRemoveButtonVisibility(card) {
        ...
        const hasData = Array.from(card.querySelectorAll('input, select, textarea')).some(function (el) {
            if (el.type === 'checkbox' || el.type === 'radio') {
                ...
                if (value === '' || value === 'Beginner' || value === 'Intermediate' || value === 'Advanced' || value === 'Expert' || value === 'Native') {
                    return false;
                }
                return true;
            } else {
                ...
                if (value === '' || value === el.placeholder ||
                    value === 'e.g., English, Spanish, French') {
                    return false;
                }
                return true;
            }
    ...
    formsContainer.addEventListener('click', function (e) {
        if (e.target.closest('.remove-entry, .remove-btn')) {
            console.log('Remove language button clicked');
            ...
            if (totalCards > 1) {
                ...
                // Show a subtle message that at least one language entry is required
    ...
    console.log('Language formset JavaScript initialization complete');
});
