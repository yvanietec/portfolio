// Handles dynamic add/remove for education formset

document.addEventListener('DOMContentLoaded', function () {
    console.log('Education formset JavaScript loaded');
    ...
    console.log('Education formset elements:', {
        addBtn: addBtn,
    ...
    if (!addBtn || !formsContainer || !totalForms) {
        console.log('Education formset elements not found');
        return;
    }
    ...
    function reindexForms() {
        console.log('Reindexing education forms');
        const cards = formsContainer.querySelectorAll('.portfolio-form-section');
        console.log('Found education cards:', cards.length);

        cards.forEach((card, idx) => {
            // Update card title
            const cardTitle = card.querySelector('.portfolio-section-title');
            if (cardTitle) cardTitle.textContent = `Education #${idx + 1}`;
    ...
    formsContainer.addEventListener('click', function (e) {
        if (e.target.closest('.remove-entry, .remove-btn')) {
            console.log('Remove education button clicked');
            ...
            if (totalCards > 1) {
                ...
                // Show a subtle message that at least one education entry is required
