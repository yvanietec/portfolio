// Handles dynamic add/remove for project formset

document.addEventListener('DOMContentLoaded', function () {
    console.log('Project formset JavaScript loaded');

    const addBtn = document.getElementById('add-project');
    const formsContainer = document.querySelector('.portfolio-form-content');
    const totalForms = document.querySelector('input[name$="-TOTAL_FORMS"]');

    console.log('Project formset elements:', {
        addBtn: addBtn,
    ...
    if (!addBtn || !formsContainer || !totalForms) {
        console.log('Project formset elements not found');
        return;
    }
    ...
    function reindexForms() {
        console.log('Reindexing project forms');
        ...
        cards.forEach((card, idx) => {
            ...
            if (cardTitle) cardTitle.textContent = `Project #${idx + 1}`;
        ...
        const hasData = Array.from(card.querySelectorAll('input, select, textarea')).some(function (el) {
            if (el.type === 'checkbox' || el.type === 'radio') {
                ...
                if (value === '' || value === el.placeholder ||
                    value === 'e.g. University of Delhi' ||
                    value === 'e.g. New Delhi, India' ||
                    value === 'e.g. Bachelor of Science in Computer Science' ||
                    value === 'Briefly describe your coursework, achievements or focus areas.' ||
                    value === 'e.g. AI-powered Resume Builder' ||
                    value === 'e.g. https://github.com/yourname/project (optional)' ||
                    value === 'e.g. Django, React, PostgreSQL' ||
                    value === 'Describe your project, your role, key achievements, tools used, etc.') {
                    return false;
                }
                return true;
            }
    ...
    formsContainer.addEventListener('click', function (e) {
        if (e.target.closest('.remove-entry, .remove-btn')) {
            ...
            if (totalCards > 1) {
                ...
                // Show a subtle message that at least one project entry is required
